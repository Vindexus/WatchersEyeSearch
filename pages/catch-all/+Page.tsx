import {
	...
	getAurasRoute, selModWeightURLSearchParams,
	selTradeLink,
	WatchersEyeContext
...
import {AURA_SLUG_MAP, AuraName} from "../../src/lib/auras";
import {auraSettingToModListString, stringToModMap} from "../../src/lib/helpers";
...
const PageInner = () => {
	...
	/*useEffect(() => {
		function onChange () {
			const currentPageUrl = '' // TODO: update for vike document.location.toString()
			const params = new URL(currentPageUrl).searchParams
			const settingsPut: AuraSettingsPut = {}
	...
	const selectedSlugs = selected.map(a => a.aura.slug)
	...
	function getAuraToggleURL (as: AuraSettings) {
		...
		if (as.enabled) {
			...
			linkAuras = [...selectedSlugs, as.aura.slug]
		}
